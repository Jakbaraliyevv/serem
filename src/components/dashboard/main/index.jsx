...
// import { toggleModal } from "../../../redux/modal-clise";
...
const BranchStats = () => {
  ...
  const [data, setData] = useState({});
  const dispatch = useDispatch();
  const refreshData = useSelector((state) => state.refreshData); // Yangi state'ni olish
  const axios = useAxios();

  // refreshData o'zgarganida API so'rovini qayta amalga oshiramiz
  useEffect(() => {
    axios({
      url: "/dashboard/",
      method: "GET",
      ...
      .then((data) => setData(data))
      .catch((error) => console.log(error));
  }, [refreshData]); // refreshData dependency sifatida qo'shildi
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {/* Modal komponentini shu yerga qo'shamiz */}
      <MyModal />
    </div>
  );
};
